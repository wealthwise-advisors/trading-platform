...
def build() -> str:
    ...
    # ── column headings ───────────────────────────────────────────────────
    o.append(f'<text x="{COL_X}" y="46" font-family="{MONO}" font-size="10.5" '
             f'letter-spacing="1.3" fill="{RETIRE}">RETIRED · 5 REPOSITORIES</text>')
    o.append(f'<text x="{HUB_X}" y="46" font-family="{MONO}" font-size="10.5" '
             f'letter-spacing="1.3" fill="{HUB}">THIS REPOSITORY</text>')
    o.append(f'<text x="{DATA_X}" y="46" font-family="{MONO}" font-size="10.5" '
    ...
    o.append(f'<text x="{GATE_X+52}" y="{gy+30:.1f}" text-anchor="middle" '
             f'font-family="{MONO}" font-size="10" font-weight="700" fill="{GATE}">'
             f'REDACTED</text>')
    o.append(f'<text x="{GATE_X+52}" y="{gy+48:.1f}" text-anchor="middle" '
             f'font-family="{FONT}" font-size="10.5" fill="{DIM}">credentials</text>')
    o.append(f'<text x="{GATE_X+52}" y="{gy+63:.1f}" text-anchor="middle" '
             f'font-family="{FONT}" font-size="10.5" fill="{DIM}">stripped</text>')
    ...
    o.append(f'<text x="{HUB_X+18}" y="{hub_y+30}" font-family="{FONT}" font-size="15" '
             f'font-weight="700" fill="{HUB}">trading-platform</text>')
    o.append(f'<text x="{HUB_X+18}" y="{hub_y+49}" font-family="{FONT}" font-size="11.5" '
    ...
    o.append(f'<g opacity="0">{fade(T_HUB)}'
             ...
             f'<text x="{HUB_X+32}" y="{ly+26}" font-family="{MONO}" font-size="12.5" '
             f'font-weight="700" fill="{DIM}">legacy/</text>'
             f'<text x="{HUB_X+32}" y="{ly+45}" font-family="{FONT}" font-size="11" '
    ...
    o.append(f'<text x="{HUB_X+32}" y="{ay+27}" font-family="{FONT}" font-size="12" '
    ...
    o.append(f'<text x="{DATA_X+16}" y="{dy+30:.1f}" font-family="{FONT}" '
             f'font-size="15" font-weight="700" fill="{DATA}">data</text>')
    o.append(f'<text x="{DATA_X+16}" y="{dy+50:.1f}" font-family="{FONT}" '
             f'font-size="11.5" fill="{DIM}">market history · Git LFS</text>')
    ...
    o.append(f'<text x="{DATA_X+28}" y="{dy+82:.1f}" font-family="{MONO}" '
             f'font-size="11" fill="{INK}">18-year 1-minute ES</text>')
    o.append(f'<text x="{DATA_X+28}" y="{dy+97:.1f}" font-family="{FONT}" '
             f'font-size="10.5" fill="{DIM}">too large for an ordinary repo</text>')
    ...
    o.append(f'<text x="{PAD}" y="{H-18}" font-family="{FONT}" font-size="11.5" '
