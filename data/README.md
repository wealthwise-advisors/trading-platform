...
| 📦 **Full archive** | [`data`](https://github.com/wealthwise-advisors/data) — 433 MB, Git LFS |
| 📁 **Path** | `data/` |
...
| [`local-demo.db`](local-demo.db) | 🧪 A throwaway database for [`run_local.py`](../scripts/run_local.py). | 398 |
...
- ➜ **`sample` is enough to run everything.** The full archive is only needed for real history.
- ➜ **`local-site` is generated.** Editing it achieves nothing — the source is [`public`](../web/public).
