 ...
 * CHART_SETUP it would prove nothing beyond that the import works.
 */
...
describe("what the panel puts on screen", () => {
  ...
  it("marks a derived row and leaves a specified one unmarked", () => {
    render(<ChartSetupPanel active={[]} />)
    expect(within(rowFor("25m")).queryByTitle(/not a specified value/i)).not.toBeNull()
    expect(within(rowFor("30m")).queryByTitle(/not a specified value/i)).toBeNull()
  })
