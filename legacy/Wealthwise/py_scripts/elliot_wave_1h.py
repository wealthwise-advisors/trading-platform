...
import talib
from assign_wave_numbers import assign_wave_numbers, update_chart
...
# st.write("This dashboard updates every 10minute to reflect new data.")
...
def fetch_data():
    ...
    # start_date_str = start_date.strftime('%Y-%m-%d')     # Convert the dates to string format (required by yfinance)    # Convert the dates to string format (required by yfinance)
    # Adjust the start_date to 00:00 (midnight) on the calculated day
    # start_date = start_date.replace(hour=0, minute=0, second=0, microsecond=0)
    # data = yf.download(ticker, start=start_date,interval='1m')
    ...
    # Define OHLC aggregation functions
    agg_funcs = {
        "Open": "first",
        "High": "max",
        "Low": "min",
    ...
    # Resample to 1-hour OHLC data
    data = data.resample('1h').agg(agg_funcs)
    data = data.dropna()
    ...
    supertrend = Supertrend(df2, 7, 3)
    df2 = df2.join(supertrend)

    return df2

# Run the update function in the background
update_chart(fetch_data, interval="1h" )