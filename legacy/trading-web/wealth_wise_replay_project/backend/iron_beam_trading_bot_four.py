...
class TradingBot:
    ...
    def __init__(self,  bot_id):
        ...
        authenticator = IronbeamAuthenticator(self.username, self.password_hash, self.api_key )
        # Authenticate and fetch the Bearer token
        token = authenticator.authenticate()
        self.order_api = OrderAPI(token)
        ...
        self.short_messages_to_check = [
            "Divergence condition met for Short"
        ]

        # Define messages to check
        self.buy_messages_to_check = [
            "Divergence condition met for Buy"
        ]
            ...
            if message["type"] == "message":
                ...
                if action == "ENABLE_LIVE_TRADING":
                    ...
                    if self.buy_order and not self.short_order:
                        self.order_api.place_market_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size)
                        stop_limit_price = float(self.get_stop_loss(True)) - float(self.stop_loss_adjust)
                        self.stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size, stop_limit_price)
                        self.insert_trade("BUY", self.current_candle.close, self.lot_size)
                    if self.short_order and not self.buy_order:
                        self.order_api.place_market_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size)
                        stop_limit_price = float(self.get_stop_loss(False)) + float(self.stop_loss_adjust)
                        self.stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size, stop_limit_price)
                ...
                # Handle Standard Trading Actions
                    ...
                    if self.buy_order:
                        ...
                        if self.live_trading and self.stop_loss_order:
                            self.order_api.cancel_order(self.username, self.stop_loss_order.order_id)
                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size)
                            self.insert_trade("SELL", self.current_candle.close, self.lot_size)
                    ...
                    if self.live_trading and self.in_trade:
                        ...
                        if self.buy_order and self.bulk_buy_stop_loss_order and self.bulk_lot_size:
                            self.order_api.cancel_order(self.username, self.bulk_buy_stop_loss_order.order_id)
                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "SELL", self.bulk_lot_size)
                            self.bulk_buy_stop_loss_order = None
                        ...
                        if self.short_order and self.bulk_sell_stop_loss_order and self.bulk_lot_size:
                            self.order_api.cancel_order(self.username, self.bulk_sell_stop_loss_order.order_id)
                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "BUY", self.bulk_lot_size)
                            self.bulk_sell_stop_loss_order = None
                ...
                # Handle BULK_BUY and BULK_SELL
                    ...
                    if lot_size > 0 and  self.live_trading and self.in_trade:
                        if self.buy_order and self.stop_loss_order:
                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "BUY", lot_size)
                            stop_limit_price = float(self.get_stop_loss(True)) - float(self.stop_loss_adjust)
                            self.bulk_buy_stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size, stop_limit_price)
                    ...
                    if lot_size > 0 and self.live_trading and self.in_trade:
                        if self.short_order and self.stop_loss_order:
                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "SELL", lot_size)
                            stop_limit_price = float(self.get_stop_loss(False)) + float(self.stop_loss_adjust)
                            self.bulk_sell_stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size, stop_limit_price)
                            self.bulk_lot_size = lot_size
    ...
    def measure_cpu_usage(self, func, *args, **kwargs):
        global method_call_count
        method_call_count += 1  # Increment counter each time the method is called
        process = psutil.Process()

        # Record CPU time before execution
        cpu_times_before = process.cpu_times()
        start_time = time.time()

        # Call the function and capture its return value(s)
        result = func(*args, **kwargs)

        # Record CPU time after execution
        end_time = time.time()
        ...
        return {
            "CPU Time Used (s)": round(cpu_time_used, 2),
            "Elapsed Time (s)": round(elapsed_time, 2),
            "CPU Usage (%)": round(cpu_usage, 2),
            "Total Calls": method_call_count  # Include the call count
        }, result  # Return both CPU stats and function results
    ...
    def get_previous_upward_swing_candle_from_10_leg(self):
        """
        Retrieves the previous upward swing candle (swing_type == 1) from the swings
        created using the zig-zag indicator for 10 legs. If only one upward swing exists,
        it returns that instead.

        Returns:
            The previous upward swing candle if available, otherwise the latest upward swing.
            Returns None if no upward swing is found.
        """
        swing_collection = self.create_swings_using_zig_zag_indicator_for_10_legs()
        ...
        return upward_swings[-2] if len(upward_swings) > 1 else upward_swings[-1]

    def get_previous_downward_swing_candle_from_10_leg(self):
        """
        Retrieves the previous downward swing candle (swing_type == -1) from the swings
        created using the zig-zag indicator for 10 legs. If only one downward swing exists,
        it returns that instead.

        Returns:
            The previous downward swing candle if available, otherwise the latest downward swing.
            Returns None if no downward swing is found.
        """
        swing_collection = self.create_swings_using_zig_zag_indicator_for_10_legs()
        ...
        return downward_swings[-2] if len(downward_swings) > 1 else downward_swings[-1]
    ...
    def add_divergence_info(self, is_buy, pre_condition, post_condition, start_candle, end_candle, is_extreme, is_rsi_13, swing_used, pre_condition_only, only_rsi, is_artificial_divergence=False):

        # if "10:25 AM" in self.current_candle.date:
        ...
        if post_condition:
            pre_condition = False
            if is_buy:
                self.add_messages("Divergence condition met for Buy")
            else:
                self.add_messages("Divergence condition met for Short")
        ...
        for divergence in existing_divergences:
            if (
                ...
                divergence.post_condition_only=post_condition
        ...
        new_divergence = DivergenceInfo(
            ...
            only_rsi=only_rsi,
            post_condition_only=post_condition,
            is_artificial_divergence = is_artificial_divergence
    ...
    def get_latest_pre_and_post_condition_divergence(self, trade_type, only_post_condition=False):
        """
        Finds the latest extreme divergence for the given trade_type.

        Args:
            trade_type: The trade type to filter divergences.
            only_post_condition (bool): If True, filters only divergences with post_condition_only=True.
        ...
        # if "10:37 AM" in self.current_candle.date:
        #     print("Put Break Point")
        ...
        # Flatten divergences list and apply filters
        valid_divergences = [
            d for d in chain.from_iterable(self.divergences_by_index.values())
            if d.trade_type == trade_type
               and (d.post_condition_only if only_post_condition else (d.pre_condition_only or d.post_condition_only))
               and (not in_trade_candle_index or d.end_candle.index >= in_trade_candle_index)
        ]
        ...
        for divergence in valid_divergences:
            if divergence.end_candle.index == current_index:
                return divergence  # Return immediately if there's an exact match

        # Otherwise, find the closest divergence
        return min(valid_divergences, key=lambda d: abs(current_index - d.end_candle.index))
    ...
    def is_artificial_bearish_divergence(self):
        ...
        if self.second_high_rsi_candle_during_buy:
            if self.current_candle.close < self.second_high_rsi_candle_during_buy.low:
                ...
                self.add_divergence_info(False, False, True,self.second_high_rsi_candle_during_buy, self.current_candle, False, False, None, False, False, True)
    ...
    def is_artificial_bullish_divergence(self):
        ...
        if self.second_low_rsi_candle_during_short:
            if self.current_candle.close > self.second_low_rsi_candle_during_short.high:
                ...
                self.add_divergence_info(True, False, True,self.second_low_rsi_candle_during_short, self.current_candle, False, False, None, False, False, True)
        ...
        if closest_lowest_rsi_candle and closest_lowest_rsi_candle.body > 0 and upward_swing_current  and not upward_swing_current.new_high_candle.divergence_used:
            ...
            if upward_swing_current.new_high_candle.close >= closest_lowest_rsi_candle.high and upward_swing_current.new_high_candle.high > closest_lowest_rsi_candle.high and time_diff.total_seconds() < 30 * 60 and time_diff.total_seconds() > 2 * 60:
                if upward_swing_current.new_high_candle.rsi > closest_lowest_rsi_candle.rsi:
                    ...
                    self.add_divergence_info(False, False, converting_to_short, closest_lowest_rsi_candle, upward_swing_current.new_high_candle, False , False, upward_swing_current, False, False )

        if closest_highest_rsi_candle and closest_highest_rsi_candle.body > 0 and upward_swing_current and not upward_swing_current.new_high_candle.divergence_used and not closest_highest_rsi_candle.divergence_used:
    ...
    def is_non_extreme_internal_bullish_divergence_for_10_leg(self, current_candle):
        ...
        if self.latest_downward_swing_candle_from_10_leg:
            ...
            if closest_lowest_rsi_candle and closest_lowest_rsi_candle.body < 0 and lowest_price_candle and not lowest_price_candle.divergence_used and not closest_lowest_rsi_candle.divergence_used:
    ...
    def get_elliot_wave_dfs(self):
        # Filter the DataFrame up to the current candle's index
        if not self.triggered_elliot_waves:
            df = self.initial_df.loc[:self.current_candle.index]
            # #df.to_csv('sample_data.csv', index=True)
            # # cpu_stats, result = self.measure_cpu_usage(ElliotWaveHelper.get_elliot_waves, df, self.current_candle.index)
            # # self.df_3legs, self.df_10legs = result
            # #print("Elliot Wave CPU Stats:", cpu_stats)
            # self.df_3legs, self.df_10legs = ElliotWaveHelper.get_elliot_waves(df, self.current_candle.index)
            # self.triggered_elliot_waves = True
            try:
                #df.to_csv('sample_data.csv', index=True)
                # cpu_stats, result = self.measure_cpu_usage(ElliotWaveHelper.get_elliot_waves, df, self.current_candle.index)
                # self.df_3legs, self.df_10legs = result
    ...
    def get_latest_match(self, df, wave_name, wave_symbols):
        df = df.loc[(df['Datetime'] <= self.current_candle.index)]
        if wave_name in df.columns:
            recent_matches = df[df[wave_name].isin(wave_symbols)]
            if not recent_matches.empty:
                return recent_matches.iloc[-1]
        return None

    def get_latest_index_for_messages(self, messages):
        """
        Finds the latest index where any of the given messages is present in `self.buying_shorting_conditions`.

        :param messages: A list of messages to search for.
        :return: The latest index containing any of the messages, or None if not found.
        """
        return max(
            (index for index in self.buying_shorting_conditions
             if any(msg in self.buying_shorting_conditions[index] for msg in messages)),
            default=None
        )

    def format_elliot_buy_sell_message(self, divergence, wave1, wave2):
        """
        Formats a message containing relevant high/low candle times along with divergence, Wave 1, and Wave 2 datetime information.

        Args:
            divergence: The divergence object to format.
            wave1: Dictionary containing 'Datetime' for Wave 1.
            wave2: Dictionary containing 'Datetime' for Wave 2.

        Returns:
            str: A formatted string message.
        """
        if not divergence:
            return "No extreme divergence found."

        # Extract Wave 1 and Wave 2 datetime details
        wave1_message = f"📈 **Wave 1 Time:** {wave1['Datetime'].strftime('%I:%M %p')}<br>"
        wave2_message = f"📉 **Wave 2 Time:** {wave2['Datetime'].strftime('%I:%M %p')}<br>"

        # Base message
        message = (
            f"📌 **Extreme Divergence Found!**<br>"
            f"🔹 **Trade Type**: {divergence.trade_type}<br>"
            f"🔹 **Divergence Candle**: {divergence.index.strftime('%I:%M %p')}<br>"
            f"📍 **Divergence Start**: {divergence.start_candle.index.strftime('%I:%M %p')}<br>"
            f"🏁 **Divergence End**: {divergence.end_candle.index.strftime('%I:%M %p')}<br>"
            f"{wave1_message}"
            f"{wave2_message}"
        )
    ...
    def check_is_converting_to_short_valid(self, converting_to_short):


        if converting_to_short: # and not extreme_condition:
            converting_to_short = False
            divergence_info = self.get_latest_pre_and_post_condition_divergence("SHORT")
            self.get_elliot_wave_dfs()
            wave1_downtrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Downtrend", ["Wave 1"])
            if wave1_downtrend_3leg is not None and wave1_downtrend_3leg['Datetime'] > divergence_info.start_candle.index:
                time_difference = abs(wave1_downtrend_3leg['Datetime'] - divergence_info.end_candle.index)
                if time_difference < timedelta(minutes=10):
                    wave2_downtrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Downtrend", ["Wave 2.1", "Wave 2.2"])
                    if wave2_downtrend_3leg is not None:
                        time_difference = wave2_downtrend_3leg['Datetime'] - wave1_downtrend_3leg['Datetime']
                        current_time_difference = self.current_candle.index - wave2_downtrend_3leg['Datetime']
                        if time_difference < timedelta(minutes=10) and current_time_difference < timedelta(minutes=10):
                            if wave2_downtrend_3leg['Datetime'] > wave1_downtrend_3leg['Datetime']:
                                if self.current_candle.close < wave1_downtrend_3leg['Low']:
                                    converting_to_short = True
                                    self.stop_loss = wave2_downtrend_3leg['High']
                                    if self.stop_loss > self.current_candle.close:
                                        self.wave1_downtrend_3leg = wave1_downtrend_3leg
                                        self.wave2_downtrend_3leg = wave2_downtrend_3leg
                                        self.add_messages(self.format_elliot_buy_sell_message(divergence_info, wave1_downtrend_3leg, wave2_downtrend_3leg))


        if self.buy_order and not self.stop_loss_order and not converting_to_short and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.wave1_uptrend_3leg is not None and self.current_candle.close > self.wave1_uptrend_3leg['High']:
                if self.stop_loss < self.current_candle.close:
                    self.add_messages("Buying Again After Stop Loss, The Current Candle Closed Above Previous Wave1 High")
                    stop_limit_price = float(self.stop_loss) - float(self.stop_loss_adjust)
                    if self.live_trading and self.enable_trade_buying:
                        self.order_api.place_market_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size)
                        self.stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size, self.stop_loss)
                        self.insert_trade("BUY", self.current_candle.close, self.lot_size)
                    self.in_trade_candle = self.current_candle
                    self.profit_taking_or_stop_loss_candle = None
                    self.close_due_to_stop_loss = False
                    self.stop_loss_used = True



        if self.buy_order and  not self.stop_loss_order and not converting_to_short and self.profit_taking_or_stop_loss_candle:
            divergence_info = self.get_latest_extreme_divergence("BUY")

            if divergence_info:
                if divergence_info.start_candle.index < self.profit_taking_or_stop_loss_candle.index:
                    divergence_info = None

            if divergence_info: # and not extreme_condition:
                self.get_elliot_wave_dfs()
                wave1_uptrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Uptrend", ["Wave 1"])
                if wave1_uptrend_3leg is not None and wave1_uptrend_3leg['Datetime'] > divergence_info.start_candle.index:
                    time_difference = abs(wave1_uptrend_3leg['Datetime'] - divergence_info.end_candle.index)
                    if time_difference < timedelta(minutes=10):
                        wave2_uptrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Uptrend", ["Wave 2.1", "Wave 2.2"])
                        if wave2_uptrend_3leg is not None:
                            if wave2_uptrend_3leg['Datetime'] > wave1_uptrend_3leg['Datetime']:
                                time_difference = wave2_uptrend_3leg['Datetime'] - wave1_uptrend_3leg['Datetime']
                                current_time_difference = self.current_candle.index - wave2_uptrend_3leg['Datetime']
                                if time_difference < timedelta(minutes=10) and current_time_difference < timedelta(minutes=10):
                                    if self.current_candle.close > wave1_uptrend_3leg['High']:
                                        self.stop_loss = wave2_uptrend_3leg['Low']
                                        self.add_messages(self.format_elliot_buy_sell_message(divergence_info, wave1_uptrend_3leg, wave2_uptrend_3leg))
                                        stop_limit_price = float(self.stop_loss) - float(self.stop_loss_adjust)
                                        if self.live_trading and self.enable_trade_buying:
                                            self.order_api.place_market_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size)
                                            self.stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size, self.stop_loss)
                                        ...
                                        self.in_trade_candle = self.current_candle
                                        self.profit_taking_or_stop_loss_candle = None
                                        self.wave2_uptrend_3leg = wave2_uptrend_3leg
                                        self.wave1_uptrend_3leg = wave1_uptrend_3leg
                                        self.stop_loss_used = False



        if converting_to_short:
            if self.in_trade_candle and self.current_candle.close < self.in_trade_candle.close and not self.profit_taking_or_stop_loss_candle:
                self.get_elliot_wave_dfs()
                wave1_uptrend_10leg = self.get_latest_match(self.df_3legs, "Wave Number Uptrend", ["Wave 2.1", "Wave 2.2"])
                if self.wave1_uptrend_3leg is not None and wave1_uptrend_10leg is not None and wave1_uptrend_10leg['Datetime'] > self.wave1_uptrend_3leg['Datetime']:
                    converting_to_short = False

        if not converting_to_short:
            if self.in_trade_candle and self.wave_w_corrective_10leg is not None and self.current_candle.close > self.in_trade_candle.close:
                self.get_elliot_wave_dfs()
                latest_wave_w_corrective_10leg = self.get_latest_match(self.df_10legs, "Corrective Pattern outer numbers", ["W"])
                if latest_wave_w_corrective_10leg is not None:
                    if latest_wave_w_corrective_10leg['Datetime'] > self.wave_w_corrective_10leg['Datetime']:
                        self.wave_w_corrective_10leg = latest_wave_w_corrective_10leg
                wave_x_corrective_10leg = self.get_latest_match(self.df_10legs, "Corrective Pattern outer numbers", ["X"])
                if wave_x_corrective_10leg is not None and wave_x_corrective_10leg['Datetime'] > self.wave_w_corrective_10leg['Datetime'] and wave_x_corrective_10leg['Datetime'] <= self.current_candle.index:
                    if self.current_candle.close < wave_x_corrective_10leg['Low']:
                        converting_to_short = True
                        self.stop_loss =  wave_x_corrective_10leg['High']
                        self.wave_x_corrective_10leg = wave_x_corrective_10leg
                        self.add_messages("Shorted Due to  W X Y Corrective Patterns")

        return converting_to_short

    def check_is_converting_to_buy_valid(self, converting_to_buy, converting_to_short=False):

        if "12:40 PM" in self.current_candle.date:
            print("Put Break Point")

        if converting_to_buy: # and not extreme_condition:
            converting_to_buy = False
            divergence_info = self.get_latest_pre_and_post_condition_divergence("BUY")
            self.get_elliot_wave_dfs()
            wave1_uptrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Uptrend", ["Wave 1"])
            if wave1_uptrend_3leg is not None and converting_to_short:
                wave1_downtrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Downtrend", ["Wave 1"])
                if wave1_downtrend_3leg['Datetime'] > wave1_uptrend_3leg['Datetime']:
                    wave1_uptrend_3leg = None
            if wave1_uptrend_3leg is not None and wave1_uptrend_3leg['Datetime'] > divergence_info.start_candle.index:
                time_difference = abs(wave1_uptrend_3leg['Datetime'] - divergence_info.end_candle.index)
                if time_difference < timedelta(minutes=10):
                    wave2_uptrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Uptrend", ["Wave 2.1", "Wave 2.2"])
        ...
        if self.short_order and not self.stop_loss_order and not converting_to_buy and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.stop_loss > self.current_candle.close:
                if self.wave1_downtrend_3leg is not None and self.current_candle.close < self.wave1_downtrend_3leg['Low']:
                    self.add_messages("Shorting Again After Stop Loss, The Current Candle Closed Below Previous Wave1 Low")
                    if self.live_trading and self.enable_trade_shorting:
                        self.order_api.place_market_order(self.username, self.symbol_ironbeam, "SELL", self.lot_size)
                        self.stop_loss_order = self.order_api.place_stop_order(self.username, self.symbol_ironbeam, "BUY", self.lot_size, self.stop_loss)
                        self.insert_trade("SELL", self.current_candle.close, self.lot_size)
        ...
        if self.short_order and not self.stop_loss_order and not converting_to_buy and self.profit_taking_or_stop_loss_candle:
            divergence_info = self.get_latest_extreme_divergence("SHORT")
            if divergence_info:
                if divergence_info.end_candle.index < self.profit_taking_or_stop_loss_candle.index:
                    divergence_info = None
            if divergence_info: # and not extreme_condition:
                self.get_elliot_wave_dfs()
                wave1_downtrend_3leg = self.get_latest_match(self.df_3legs, "Wave Number Downtrend", ["Wave 1"])
        ...
        if converting_to_buy:
            if self.in_trade_candle and self.current_candle.close > self.in_trade_candle.close and not self.profit_taking_or_stop_loss_candle:
                self.get_elliot_wave_dfs()
                wave1_downtrend_10leg = self.get_latest_match(self.df_3legs, "Wave Number Downtrend", ["Wave 2.1", "Wave 2.2"])
                if self.wave1_downtrend_3leg is not None and wave1_downtrend_10leg is not None and wave1_downtrend_10leg['Datetime'] > self.wave1_uptrend_3leg['Datetime']:
                    converting_to_buy = False

        if converting_to_buy:
            self.get_elliot_wave_dfs()
            if self.wave_w_corrective_10leg is None :
                wave_w_corrective_10leg = self.get_latest_match(self.df_10legs, "Corrective Pattern outer numbers", ["W"])
                if self.in_trade_candle and wave_w_corrective_10leg is not None and wave_w_corrective_10leg['Datetime'] > self.in_trade_candle.index:
                    self.wave_w_corrective_10leg  = wave_w_corrective_10leg
            if self.wave_x_corrective_10leg is not None:
                converting_to_buy = False


        if not converting_to_buy:
            if self.in_trade_candle and self.wave_x_corrective_10leg is not None and self.current_candle.close < self.in_trade_candle.close:
                self.get_elliot_wave_dfs()
                if self.wave_x_corrective_10leg is not None:
                    latest_wave_x_corrective_10leg = self.get_latest_match(self.df_10legs, "Corrective Pattern outer numbers", ["X"])
                    if latest_wave_x_corrective_10leg is not None and latest_wave_x_corrective_10leg['Datetime'] > self.wave_x_corrective_10leg['Datetime']:
                        self.wave_x_corrective_10leg = latest_wave_x_corrective_10leg
                wave_y_corrective_10leg = self.get_latest_match(self.df_10legs, "Corrective Pattern outer numbers", ["Y"])
                if wave_y_corrective_10leg is not None and wave_y_corrective_10leg['Datetime'] > self.wave_x_corrective_10leg['Datetime'] and wave_y_corrective_10leg['Datetime'] <= self.current_candle.index:
                    if self.current_candle.close > wave_y_corrective_10leg['High']:
                        converting_to_buy = True
                        self.stop_loss =  wave_y_corrective_10leg['Low']
                        self.add_messages("Buy Due to  W X Y Corrective Patterns")
        ...
        while running:
            try:
                ...
                if self.stop_loss_order:
                    self.stop_loss_order = self.order_api.get_order_by_order_id(self.username, self.stop_loss_order.order_id, "ANY")
                    if self.stop_loss_order and self.stop_loss_order.status == "CANCELLED":
                    ...
                    if self.stop_loss_order and self.stop_loss_order.status == "FILLED":
                        logging.info(f"{self.symbol_ironbeam} :Stop Loss with order Id # {self.stop_loss_order.order_id} is Filled")
                        self.stop_loss_order = None
                        self.profit_taking_or_stop_loss_candle = self.current_candle
                        self.close_due_to_stop_loss = True
                        logging.info("Stop Loss Order Exceuted By Broker")
                ...
                self.previous_downward_swing_candle_from_10_leg = self.get_previous_downward_swing_candle_from_10_leg()
                self.previous_upward_swing_candle_from_10_leg = self.get_previous_upward_swing_candle_from_10_leg()
                ...
                if not self.in_trade:
                    ...
                    # if "09:38 AM" in self.current_candle.date:
                    #     print("Put Break Point")
                    ...
                    if not converting_to_buy:
                        ...
                        if divergence_info and not self.is_extreme_violation(divergence_info):
                            ...
                            if time_difference < timedelta(minutes=30):
                                if self.current_candle.close > divergence_info.end_candle.high:
                                    converting_to_buy = True
                                    divergence_info.end_candle.divergence_used = True
                                    self.real_buy_divergence_candle = divergence_info
                                    ...
                                    self.add_helper_messages(f"Divergence condition met for Buy")
                    ...
                    if not converting_to_short:
                        ...
                        if divergence_info and not self.is_extreme_violation(divergence_info):
                            time_difference = abs(divergence_info.start_candle.index - divergence_info.end_candle.index)
                            if time_difference < timedelta(minutes=30):
                                if self.current_candle.close < divergence_info.end_candle.low:
                                    converting_to_short = True
                                    self.real_short_divergence_candle = divergence_info
                                    divergence_info.end_candle.divergence_used = True
                                    self.update_divergence_used_in_memory(divergence_info.end_candle)
                                    self.short_divergence_candle = self.get_candle_by_index(divergence_info.end_candle.index)
                                    self.add_helper_messages(f"Divergence condition met for Short")
                    ...
                    if not short_condition:
                        ...
                        if divergence_info and not self.is_extreme_violation(divergence_info):
                            time_difference = abs(divergence_info.start_candle.index - divergence_info.end_candle.index)
                            if time_difference < timedelta(minutes=30):
                                if self.current_candle.close < divergence_info.end_candle.low and self.current_candle.body < 0:
                                    short_condition = True
                                    self.real_short_divergence_candle = divergence_info
                                    divergence_info.end_candle.divergence_used = True
                                    self.update_divergence_used_in_memory(divergence_info.end_candle)
                    ...
                    if not buy_condition:
                        ...
                        if divergence_info and not self.is_extreme_violation(divergence_info):
                            time_difference = abs(divergence_info.start_candle.index - divergence_info.end_candle.index)
                            if time_difference < timedelta(minutes=30):
                                if self.current_candle.close > divergence_info.end_candle.high and self.current_candle.body > 0:
                                    buy_condition = True
                                    self.real_buy_divergence_candle = divergence_info
                                    divergence_info.end_candle.divergence_used = True
                                    self.update_divergence_used_in_memory(divergence_info.end_candle)
                    ...
                    short_condition = self.check_is_converting_to_short_valid(short_condition)
                    buy_condition = self.check_is_converting_to_buy_valid(buy_condition, short_condition)
                ...
                if self.in_trade:
                    ...
                    if self.buy_order:
                        ...
                        if "01:46 PM" in self.current_candle.date:
                            print("Put Break Point")
                        ...
                        if not converting_to_short:
                            ...
                            if converting_to_short:
                                ...
                                self.add_helper_messages(f"Artificial Divergence condition met for Short")
                            ...
                            if self.in_memory_short_divergence_candle:
                                if self.current_candle.close < self.in_memory_short_divergence_candle.low:
                                    if self.buy_divergence_candle and not self.buy_divergence_candle.is_extreme_buy_sell:
                                        ...
                                        self.add_divergence_info(False, False, True, self.in_memory_short_divergence_candle, self.current_candle, False, False, None, False, False, True)
                                    if  self.buy_divergence_candle and self.buy_divergence_candle.is_extreme_buy_sell:
                                        if self.current_candle.close < self.buy_divergence_candle.low:
                                            ...
                                            self.add_helper_messages(f"Artificial Divergence condition met for Short")
                                            self.short_divergence_candle = self.in_memory_short_divergence_candle
                                            self.add_divergence_info(False, False, True, self.buy_divergence_candle, self.current_candle, False, False, None, False, False, True)
                        ...
                        if self.buy_divergence_candle and not converting_to_short:
                            if self.current_candle.close < self.buy_divergence_candle.low:
                                ...
                                self.add_messages("Artificial Divergence condition met for Short")
                                self.add_divergence_info(False, False, True,self.buy_divergence_candle, self.current_candle, False, False, None, False, False, True)
                        ...
                        if not converting_to_short and self.first_high_rsi_candle_during_buy and self.first_high_rsi_candle_during_buy.index > self.in_trade_candle.index:
                            if self.current_candle.close < self.first_high_rsi_candle_during_buy.low and self.current_candle.rsi< 5:
                                ...
                                self.add_divergence_info(False, False, True, self.first_high_rsi_candle_during_buy, self.current_candle, False, False, None, False, False, True)

                        converting_to_short = self.check_is_converting_to_short_valid(converting_to_short)
                        if converting_to_short:
                            sell_condition = True
                            buy_condition = False
                            stop_loss = self.get_stop_loss(False)
                    ...
                    if self.short_order:
                        ...
                        if not converting_to_buy:
                            ...
                            if converting_to_buy:
                                ...
                                self.add_helper_messages(f"Artificial Divergence condition met for Buy")
                            ...
                            if self.in_memory_buy_divergence_candle:
                                if self.current_candle.close > self.in_memory_buy_divergence_candle.high:
                                    if self.short_divergence_candle and not self.short_divergence_candle.is_extreme_buy_sell:
                                        ...
                                        self.add_divergence_info(True, False, True, self.in_memory_buy_divergence_candle, self.current_candle, False, False, None, False, False, True)
                        ...
                        if self.short_divergence_candle and not converting_to_buy:
                            if self.current_candle.close > self.short_divergence_candle.high:
                                ...
                                self.add_messages("Artificial Divergence condition met for Buy")
                                self.add_divergence_info(True, False, True,self.short_divergence_candle, self.current_candle, False, False, None, False, False, True)
                        ...
                        if not converting_to_buy and self.first_low_rsi_candle_during_short and self.first_low_rsi_candle_during_short.index > self.in_trade_candle.index:
                            if self.current_candle.close > self.first_low_rsi_candle_during_short.high and self.current_candle.rsi > 94:
                                ...
                                self.add_helper_messages(f"Artificial Divergence condition met for Buy")
                                self.add_divergence_info(True, False, True, self.first_low_rsi_candle_during_short, self.current_candle, False, False, None, False, False, True)


                        converting_to_buy = self.check_is_converting_to_buy_valid(converting_to_buy)
