...
def check_for_trading(row):
    ...
    if is_between('07:00', '15:45', target_time):
        return 1
    else:
        return 0
...
def inject_zoom_persistence(html_file, plot_id):
    ...
    refresh_meta_tag = '<meta http-equiv="refresh" content="3000000000">'
...
def get_history_candles(start_time_string, end_time_string, bar_type_period):
    data_manager = DataManager()
    logging.info(f"start_time_string # {start_time_string} , end_time_string # {end_time_string}")
    df = data_manager.download_historical_tick_data(start_time_string, end_time_string, bar_type_period)
...
if __name__ == "__main__":
    ...
    start_time_string = start_date + ' ' +  start_time + ':00'
    end_time_String = end_date + ' '+ end_time + ':00'
    frequencyType, frequency, resample_interval = get_frequency_settings(interval)
    df = get_history_candles(start_time_string, end_time_String, 1)
