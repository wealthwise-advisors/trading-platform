...
class TradingBot:
    ...
    def create_rsi_stoch_fib(self, candle):
        ...
        data.body_volume = float(candle.BODY_VOLUME)
        data.previous_body_volume = float(candle.PREVIOUS_BODY_VOLUME)
    ...
    def format_extreme_divergence_message(self, divergence, trade_failed=False):
        ...
        candle_message = "\n".join(candle_details) if candle_details else "No relevant candles found."

        # Base message
        message = (
            f"**Extreme Divergence Found!**<br>"
            f"**Trade Type**: {divergence.trade_type} \n"
            f"**Divergence Candle**: {divergence.index.strftime('%I:%M %p')} \n"
            f"**Divergence Start**: {divergence.start_candle.index.strftime('%I:%M %p')}, **RSI**: {divergence.start_candle.rsi} \n"
            f"**Divergence End**: {divergence.end_candle.index.strftime('%I:%M %p')}, **RSI**: {divergence.end_candle.rsi} \n"
            f"**Relevant Candle Times & Indicators:**<br>{candle_message}"
        )

        # Append additional message if trade failed
        if trade_failed:
            message += "\n **The trade went wrong and exited with a loss.**"
    ...
    def run(self):
        ...
        highest_price_row = data_high_low.loc[data_high_low['High'].idxmax()]
        lowest_price_row = data_high_low.loc[data_high_low['Low'].idxmin()]
        for i in range(-60, 0):
        ...
        while running:
            ...
            logging.info(f"Current Trade Status # {self.in_trade} ,  Buy Trade # {self.buy_order} , Short order {self.short_order} , Live Trading {self.live_trading}")
