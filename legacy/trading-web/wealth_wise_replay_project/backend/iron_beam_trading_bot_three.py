...
class TradingBot:
    ...
    def __init__(self,  bot_id):
        ...
        self.back_testing_date = '2024-01-29'
        self.bot_sleep_seconds  = 60
        self.step_forward = 1
        self.end_time = None
    ...
    def submit_market_order(self, is_buy, lot_size):
        ...
        if is_buy:
            ...
            order_type = 'short'
        self.market_order = self.trade_manager.place_order(
            order_id=order_id, order_type=order_type, symbol=self.symbol_schwab, entry_price=self.current_candle.close, quantity=lot_size)

        logging.info(f"Successfully submitted market order {self.market_order}, order_id # {order_id}")

    def submit_stop_market_order(self, stop_loss_price):
        order_id = '{0}_stop_mkt_order'.format(datetime.now().strftime('%Y%m%d_%H%M%S_%f')[:-3])
        stop_loss_order = self.trade_manager.place_stop_loss(
            stop_loss_id=order_id, linked_order_id=self.market_order.order_id, symbol=self.symbol_schwab, stop_loss_price=stop_loss_price)

        logging.info(f"{self.current_candle.date} : Stop market order submitted {stop_loss_order}")
                ...
                # Handle Standard Trading Actions
                    ...
                    if self.buy_order:
                        ...
                        if self.live_trading and self.stop_loss_order:
                            self.trade_manager.update_stop_loss_status(self.stop_loss_order.order_id, "CANCELLED")
                            self.trade_manager.close_order(self.stop_loss_order.linked_order_id, self.current_candle.close)
                        ...
                        if self.live_trading:
                            ...
                            self.insert_trade("SELL", self.current_candle.close, self.lot_size)
                            self.stop_loss_order = self.submit_stop_market_order(stop_limit_price)
                    ...
                    if self.live_trading and self.in_trade:
                        ...
                        if self.buy_order and self.bulk_buy_stop_loss_order and self.bulk_lot_size:
                            self.trade_manager.update_stop_loss_status(self.stop_loss_order.order_id, "CANCELLED")
                            self.trade_manager.close_order(self.bulk_buy_stop_loss_order.linked_order_id, self.current_candle.close)
                            self.bulk_buy_stop_loss_order = None
                ...
                # Handle BULK_BUY and BULK_SELL
                    ...
                    if lot_size > 0 and  self.live_trading and self.in_trade:
                        if self.buy_order and self.stop_loss_order:
                            self.submit_market_order(True,self.lot_size)
                            stop_limit_price = float(self.get_stop_loss(True)) - float(self.stop_loss_adjust)
                            self.bulk_buy_stop_loss_order = self.submit_stop_market_order(stop_limit_price)
                    ...
                    if lot_size > 0 and self.live_trading and self.in_trade:
                        if self.short_order and self.stop_loss_order:
                            self.submit_market_order(False,self.lot_size)
                            stop_limit_price = float(self.get_stop_loss(False)) + float(self.stop_loss_adjust)
                            self.bulk_sell_stop_loss_order = self.submit_stop_market_order(stop_limit_price)
                            self.bulk_lot_size = lot_size
                    ...
                    logging.info(f"Bot {self.bot_id}: Executing MOVE_FORWARD with {minutes} minutes")
                    self.end_time += timedelta(minutes=minutes-1)
    ...
    def get_history_candles(self, start_time_string, end_time_string, bar_type_period):
        logging.info(f"Getting historical data from start_time {start_time_string} and end_time {end_time_string} and bar_type_period {bar_type_period}")
        df = self.data_manager.download_historical_tick_data(start_time_string, end_time_string, bar_type_period)
        return df
    ...
    def run(self):
        ...
        start_time = self.back_testing_date + ' 00:01:00'
        end_time = self.back_testing_date + ' 07:00:00'

        data = self.get_history_candles(start_time, end_time, 1)
        ...
        self.end_time = datetime.strptime(f"{self.back_testing_date} 07:00:00", "%Y-%m-%d %H:%M:%S")
        trading_end_time = self.end_time.replace(hour=10, minute=00)
        while self.end_time <= trading_end_time:
            try:

                if self.market_order:
                    self.trade_manager.update_profit_for_candle(self.market_order.order_id, self.current_candle.close)
                    self.trade_manager.show_summary()

                bot_start_time = time.time()
                next_run_time += timedelta(seconds=self.bot_sleep_seconds)
                buy_condition = False
                ...
                converting_to_short = False
                converting_to_buy = False
                self.end_time += timedelta(minutes=self.step_forward)
                interval_minutes = 1
                num_candles = 500
                total_time_span = timedelta(minutes=interval_minutes * num_candles)
                start_time = self.end_time - total_time_span
                end_time_string = self.end_time.strftime("%Y-%m-%d %H:%M:%S")
                start_time_string = start_time.strftime("%Y-%m-%d %H:%M:%S")
                data = self.get_history_candles(start_time_string, end_time_string, 1)
                #data = self.remove_last_candle_if_invalid(df)
                self.initial_df = data.copy()
                self.initial_df.index = pd.to_datetime(self.initial_df.index)
                self.initial_df.index = self.initial_df.index.tz_localize(None)
                ...
                if self.stop_loss_order:
                    self.stop_loss_order = self.trade_manager.get_order(self.stop_loss_order.order_id)
