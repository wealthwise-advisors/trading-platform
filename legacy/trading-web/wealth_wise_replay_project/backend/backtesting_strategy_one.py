...
def apply_rsi_stoch_doji(df):
    ...
    df["Avg_Close"] = df["Close"].rolling(window=8).mean()

    # Calculate the rolling standard deviation of close prices over 10 candles
    df["Std_Close"] = df["Close"].rolling(window=8).std()
...
class RSIStochExtremeStrategy(Strategy):
    ...
    def create_rsi_stoch_fib(self):
        ...
        data.std_second_top_price = self.data.Two_SD_Top[-1]
        data.std_second_bottom_price = self.data.Two_SD_Bottom[-1]
        return data
    ...
    def get_highest_rsi_candle_above_94_between_dates(self, start_datetime, end_datetime):
        ...
        filtered_candles_above_94 = [candle for candle in filtered_candles if candle.rsi > self.high_rsi_limit]
    ...
    def get_lowest_rsi_candle_below_5_between_dates(self, start_datetime, end_datetime):
        ...
        filtered_candles_below_5 = [candle for candle in filtered_candles if candle.rsi < self.low_rsi_limit]
    ...
    def add_divergence_info(self, is_buy, pre_condition, post_condition, start_candle, end_candle, is_extreme, is_rsi_13, swing_used, pre_condition_only):
        ...
        if start_candle.rsi >= self.high_rsi_limit or end_candle.rsi >= self.high_rsi_limit:
            is_extreme = True

        if start_candle.rsi <= self.low_rsi_limit or end_candle.rsi <= self.low_rsi_limit:
            is_extreme = True
    ...
    def get_latest_extreme_divergence(self, trade_type):
        ...
        Returns:
            The extreme divergence that is closest to the current_candle.index.
            Returns None if no extreme divergences exist for the given trade_type.
        """
        current_index = self.current_candle.index
        closest_divergence = None
        min_distance = float('inf')

        # Iterate over all divergences
        for divergences in self.divergences_by_index.values():
            for divergence in divergences:
                if divergence.trade_type == trade_type:
                    # Convert Timedelta to seconds for comparison
                    distance = abs((current_index - divergence.start_candle.index).total_seconds())

                    if distance < min_distance:
                        min_distance = distance
                        closest_divergence = divergence

        return closest_divergence
        ...
        if closest_lowest_rsi_candle and closest_lowest_price_candle and not closest_lowest_price_candle.divergence_used:
            if closest_lowest_price_candle.index > closest_lowest_rsi_candle.index:
                if closest_lowest_price_candle.close < closest_lowest_rsi_candle.close and closest_lowest_price_candle.close < closest_lowest_rsi_candle.low:
                    ...
                    is_extreme = closest_lowest_rsi_candle.rsi <= self.low_rsi_limit or closest_lowest_price_candle.rsi <= self.low_rsi_limit
                    self.add_divergence_info(True, pre_condition_buy, converting_to_buy, closest_lowest_rsi_candle, closest_lowest_price_candle, is_extreme , False, None, False )
                ...
                if closest_lowest_price_candle.index > closest_lowest_rsi_candle.index:
                    if closest_lowest_price_candle.close < closest_lowest_rsi_candle.close and closest_lowest_price_candle.close < closest_lowest_rsi_candle.low:
                        ...
                        if converting_to_buy:
                            ...
                            self.update_divergence_used_in_memory(closest_lowest_price_candle)
                        is_extreme = closest_lowest_rsi_candle.rsi <= self.low_rsi_limit or closest_lowest_price_candle.rsi <= self.low_rsi_limit
                        self.add_divergence_info(True, pre_condition_buy, converting_to_buy, closest_lowest_rsi_candle, closest_lowest_price_candle, is_extreme , True, None, False )
        ...
        if closest_highest_rsi_candle and closest_highest_price_candle and not closest_highest_price_candle.divergence_used:
            if closest_highest_price_candle.index > closest_highest_rsi_candle.index:
                if closest_highest_price_candle.close > closest_highest_rsi_candle.close and closest_highest_price_candle.close > closest_highest_rsi_candle.high:
                    ...
                    is_extreme = closest_highest_price_candle.rsi >= self.high_rsi_limit or closest_highest_rsi_candle.rsi >= self.high_rsi_limit
                    self.add_divergence_info(False, pre_condition_short, converting_to_short, closest_highest_rsi_candle, closest_highest_price_candle, is_extreme , False, None, False )
    ...
    def all_low_exist_and_after_index_for_short(self, threshold_index, check_rsi_only=False):
        """
        Checks if all required low candles exist and are after the given threshold_index.
        If check_rsi_only=True, it only checks for RSI low.
        Otherwise, RSI and SD must match, and at least one of K or D should match.

        Args:
            threshold_index (Timestamp): The index threshold.
            check_rsi_only (bool): Whether to check only RSI low.
        ...
        if check_rsi_only:
            # Check only RSI
            rsi_obj = getattr(self, "candle_touched_rsi_low", None)
            return rsi_obj is not None and rsi_obj.index > threshold_index

        # Mandatory attributes: RSI and SD must match
        rsi_obj = getattr(self, "candle_touched_rsi_low", None)
        sd_obj = getattr(self, "candle_touched_sd_low", None)

        if rsi_obj is None or rsi_obj.index <= threshold_index:
            return False
        if sd_obj is None or sd_obj.index <= threshold_index:
            return False

        # Optional attributes: At least one of K or D must match
        k_obj = getattr(self, "candle_touched_k_low", None)
        d_obj = getattr(self, "candle_touched_d_low", None)

        if (k_obj and k_obj.index > threshold_index) or (d_obj and d_obj.index > threshold_index):
            return True  # At least one optional condition met

        return False  # If RSI and SD match but neither K nor D does

    def all_high_exist_and_after_index_for_buy(self, threshold_index, check_rsi_only=False):
        """
        Checks if all required high candles exist and are after the given threshold_index.
        If check_rsi_only=True, it only checks for RSI high.
        Otherwise, RSI and SD must match, and at least one of K or D should match.

        Args:
            threshold_index (Timestamp): The index threshold.
            check_rsi_only (bool): Whether to check only RSI high.
        ...
        if check_rsi_only:
            # Check only RSI
            rsi_obj = getattr(self, "candle_touched_rsi_high", None)
            return rsi_obj is not None and rsi_obj.index > threshold_index

        # Mandatory attributes: RSI and SD must match
        rsi_obj = getattr(self, "candle_touched_rsi_high", None)
        sd_obj = getattr(self, "candle_touched_sd_high", None)
        ...
        # Optional attributes: At least one of K or D must match
        k_obj = getattr(self, "candle_touched_k_high", None)
        d_obj = getattr(self, "candle_touched_d_high", None)
    ...
    def format_extreme_divergence_message(self, divergence, trade_failed=False):
        """
        Formats a message containing relevant high/low candle times along with divergence start and end candle times.

        Args:
            divergence: The divergence object to format.
            trade_failed (bool): If True, adds an additional message about trade failure.
        ...
        # Attribute name mapping to avoid special characters
        indicator_mapping = {
            "RSI": "rsi",
            "Stoch %K": "stoch_k",
            "Stoch %D": "stoch_d",
        ...
        # Select relevant candles based on trade type
        if divergence.trade_type == "SHORT":
            relevant_candles = {
                "RSI Low": (self.candle_touched_rsi_low, "RSI"),
                "SD Low": (self.candle_touched_sd_low, "Std Bottom Price"),
                "K Low": (self.candle_touched_k_low, "Stoch %K"),
                "D Low": (self.candle_touched_d_low, "Stoch %D"),
            }
        else:  # BUY trade type
            relevant_candles = {
                "RSI High": (self.candle_touched_rsi_high, "RSI"),
                "SD High": (self.candle_touched_sd_high, "Std Top Price"),
                "K High": (self.candle_touched_k_high, "Stoch %K"),
                "D High": (self.candle_touched_d_high, "Stoch %D"),
            }

        # Extract relevant candle times with appropriate indicators
        candle_details = []
        for label, (candle, indicator) in relevant_candles.items():
            if candle:
                if indicator in indicator_mapping:
                    value = getattr(candle, indicator_mapping[indicator])
                    candle_details.append(f"{label}: {candle.index.strftime('%I:%M %p')}, **{indicator}**: {value}")
                elif "Std" in indicator:  # Standard deviation price handling
                    std_price = round(getattr(candle, "std_second_top_price" if "Top" in indicator else "std_second_bottom_price"), 2)
                    candle_details.append(f"{label}: {candle.index.strftime('%I:%M %p')}, **{indicator}**: {std_price}")
                else:
                    candle_details.append(f"{label}: {candle.index.strftime('%I:%M %p')}")

        candle_message = "<br>".join(candle_details) if candle_details else "No relevant candles found."
        ...
        message = (
            ...
            f"📍 **Divergence Start**: {divergence.start_candle.index.strftime('%I:%M %p')}, **RSI**: {divergence.start_candle.rsi}<br>"
            f"🏁 **Divergence End**: {divergence.end_candle.index.strftime('%I:%M %p')}, **RSI**: {divergence.end_candle.rsi}<br>"
            f"📊 **Relevant Candle Times & Indicators:**<br>{candle_message}"
        )

        # Append additional message if trade failed
        if trade_failed:
            message += "<br>⚠️ **The trade went wrong and exited with a loss.**"
    ...
    def check_is_converting_to_short_valid(self, converting_to_short):
        ...
        converting_to_short = False
        if divergence_info:
            converting_to_short = self.all_low_exist_and_after_index_for_short(divergence_info.start_candle.index)
            if converting_to_short:
                self.add_messages(self.format_extreme_divergence_message(divergence_info))

        # if not converting_to_short and self.high_sd_rsi_k_green_candle:
        #     if self.in_trade_candle and self.current_candle.close < self.in_trade_candle.close:
        #         if self.current_candle.close < self.high_sd_rsi_k_green_candle.low:
        #             converting_to_short = True
        #             self.add_messages(f"The Trade went wrong and closed below previous Trade SD High candle {self.high_sd_rsi_k_green_candle.index.strftime('%I:%M %p')}")

        if not converting_to_short:
            if self.in_trade_candle and self.current_candle.close < self.in_trade_candle.close:
                if self.current_candle.close < self.in_trade_candle.low:
                    converting_to_short = self.all_low_exist_and_after_index_for_short(self.in_trade_candle.index, True)
                    if converting_to_short:
                        self.add_messages(f"The Trade went wrong and closed below lowest rsi candle {self.candle_touched_rsi_low.index.strftime('%I:%M %p')}")


        return converting_to_short

    def check_is_converting_to_buy_valid(self, converting_to_buy):

        if "10:50 AM" in self.current_candle.date:
            print("Put Break Point")

        divergence_info = self.get_latest_extreme_divergence("BUY")
        converting_to_buy = False
        if divergence_info:
            converting_to_buy = self.all_high_exist_and_after_index_for_buy(divergence_info.start_candle.index)
            if converting_to_buy:
                self.add_messages(self.format_extreme_divergence_message(divergence_info))


        # if not converting_to_buy and self.low_sd_rsi_k_red_candle:
        #     if self.in_trade_candle  and self.current_candle.close > self.in_trade_candle.close:
        #         if self.current_candle.close > self.low_sd_rsi_k_red_candle.high:
        #             converting_to_buy = True
        #             self.add_messages(f"The Trade went wrong and closed above  previous Trade SD Low candle {self.low_sd_rsi_k_red_candle.index.strftime('%I:%M %p')}")

        if not converting_to_buy:
            if self.in_trade_candle and self.current_candle.close > self.in_trade_candle.close:
                if self.current_candle.close > self.in_trade_candle.high:
                    converting_to_buy = self.all_high_exist_and_after_index_for_buy(self.in_trade_candle.index, True)
                    if converting_to_buy:
                        self.add_messages(f"The Trade went wrong and closed above highest rsi candle {self.candle_touched_rsi_high.index.strftime('%I:%M %p')}")
    ...
    def next(self):
        ...
        if self.enable_trade_shorting and not self.enable_trade_buying:
            if self.current_candle.rsi < self.high_rsi_limit and self.current_candle.stoch_k < 100:
                if self.current_candle.previous_rsi > self.high_rsi_limit and self.current_candle.previous_stoch_k > self.high_k_limit:
        ...
        if self.enable_trade_buying and not self.enable_trade_shorting:
            if self.current_candle.rsi > self.low_rsi_limit and self.current_candle.stoch_k > 0:
                if self.current_candle.previous_rsi < self.low_rsi_limit and self.current_candle.previous_stoch_k < self.low_k_limit:
        ...
        if self.data.TRADE[-1] == 1:
            ...
            if not self.in_trade:
                ...
                buy_condition = self.check_is_converting_to_buy_valid(buy_condition)
            ...
            if self.in_trade:
                ...
                if self.current_candle.fib == 68:
                    print("Put Break Point")

                if self.buy_order:

                    if "12:18 PM" in self.current_candle.date:
                        print("Put Break Point")
                    ...
                    if self.enable_trade_shorting and not self.enable_trade_buying:
                        if self.current_candle.previous_rsi > self.high_rsi_limit and self.current_candle.rsi < self.high_rsi_limit:
                            self.short_triggered_candle_two = current_candle
                            self.sell_stop_limit = None
                ...
                if self.short_order:
                    ...
                    if self.enable_trade_buying and not self.enable_trade_shorting:
                        if self.current_candle.previous_rsi < self.low_rsi_limit and self.current_candle.rsi > self.low_rsi_limit:
                            self.buy_triggered_candle_two = current_candle
                            self.cover_stop_limit = None
...
def calculate_profit(row):
    profit = row.Final - 1000000
    return profit

def get_previous_working_day(given_date_str):
    ...
    # Check if the previous day is a Saturday or a holiday (Sunday is considered a working day)
    while previous_day.weekday() == 5 or previous_day.strftime("%Y-%m-%d") in holidays:
        previous_day -= timedelta(days=1)

    # Format the previous working day as a string
    previous_working_day_str = previous_day.strftime("%Y-%m-%d")

    return previous_working_day_str

def apply_fib_text(row, fib_generated_from):
    return f"{row.CLOSE_OPEN_OPEN}={row.Fib_Number}={fib_generated_from}={row.CHART_TYPE}"

def apply_green_wick_up_down_text(row):
    return f"{row.GREEN_WICK_UP} / {row.GREEN_WICK_DOWN}"

def apply_red_wick_up_down_text(row):
    return f"{row.RED_WICK_UP} / {row.RED_WICK_DOWN}"

def apply_green_wick_volume_up_down_text(row):
    return f"{row.GREEN_WICK_UP_VOLUME} / {row.GREEN_WICK_DOWN_VOLUME}"

def apply_red_wick_volume_up_down_text(row):
    return f"{row.RED_WICK_UP_VOLUME} / {row.RED_WICK_DOWN_VOLUME}"

def get_previous_working_day_date(given_date):
    # List of holidays in 2023
    holidays = [
        # 2022 Holidays
        "2022-01-01",  # New Year's Day
...
def find_start_day_by_interval(end_date, interval):
    # Dictionary to map intervals to timedelta values
    interval_days = {
        '3T': 3,
        '5T': 5,
        '8T': 5,
    ...
    # Get the corresponding timedelta for the interval, return None if not found
    days_to_subtract = interval_days.get(interval)
    if days_to_subtract is None:
        return None

    # Calculate and return the start date
    start_date = end_date - timedelta(days=days_to_subtract)
    start_date = get_previous_working_day_date(start_date)
    return start_date
...
def inject_zoom_persistence(html_file, plot_id):
    ...
    refresh_meta_tag = '<meta http-equiv="refresh" content="60">'
...
if __name__ == "__main__":
    ...
    logging.info(f"output_html {output_html}")
    if len(df) > 0:
