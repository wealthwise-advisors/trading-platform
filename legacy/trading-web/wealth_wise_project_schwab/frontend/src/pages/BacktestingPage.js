...
export default function BacktestingPage() {
  const getCurrentDate = () => {
    return new Date().toISOString().split("T")[0]; // Formats YYYY-MM-DD
  };
  ...
  const [params, setParams] = useState({
    ...
    end_time: "14:45", // Default end time
    interval: "1m",
    lot_size: 500,
    stop_loss_adjust: 10,
...
const handleChange = (e) => {
  ...
  setParams((prev) => ({
    ...
    [name]:
      type === "checkbox"
        ? checked
        : name === "lot_size" || name === "stop_loss_adjust"
        ? parseFloat(value) || 0
        : value,
  }));
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block font-semibold mb-2">
            <input
              type="checkbox"
              name="enable_trade_buying"
              checked={params.enable_trade_buying}
              onChange={handleChange}
              className="mr-2"
            />
            Enable Trade Buying
        ...
        <div>
          <label className="block font-semibold mb-2">
            <input
              type="checkbox"
              name="enable_trade_shorting"
              checked={params.enable_trade_shorting}
              onChange={handleChange}
              className="mr-2"
            />
            Enable Trade Shorting
          </label>
        </div>
