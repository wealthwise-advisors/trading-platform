...
class TradingBot:
        ...
        for message in pubsub.listen():
            if message["type"] == "message":
                ...
                if action == "ENABLE_LIVE_TRADING":
                    ...
                    #     self.order_api.place_market_order(True, self.lot_size)
    ...
    def check_is_converting_to_short_valid(self, converting_to_short):
        ...
        if self.buy_order and not self.stop_loss_order_id and not converting_to_short and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.wave1_uptrend_3leg is not None and self.current_candle.close > self.wave1_uptrend_3leg['High']:
                if self.stop_loss < self.current_candle.close:
                    ...
                    if self.live_trading and self.enable_trade_buying:
                        self.order_api.submit_buy_sell_market_order(True, self.lot_size)
                        self.order_api.submit_stop_market_order(self.stop_loss, False,  self.lot_size)
                        self.stop_loss_order_id = self.order_api.stop_loss_order_id
                        self.insert_trade("BUY", self.current_candle.close, self.lot_size)
                    self.in_trade_candle = self.current_candle
                    self.profit_taking_or_stop_loss_candle = None
        ...
        if self.buy_order and  not self.stop_loss_order_id and not converting_to_short and self.profit_taking_or_stop_loss_candle:
    ...
    def check_is_converting_to_buy_valid(self, converting_to_buy, converting_to_short=False):
        ...
        if self.short_order and not self.stop_loss_order_id and not converting_to_buy and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.stop_loss > self.current_candle.close:
                if self.wave1_downtrend_3leg is not None and self.current_candle.close < self.wave1_downtrend_3leg['Low']:
                    ...
                    if self.live_trading and self.enable_trade_shorting:
                        self.order_api.submit_short_cover_market_order(True, self.lot_size)
                        self.order_api.submit_stop_market_order(self.stop_loss, True,  self.lot_size)
                        self.stop_loss_order_id = self.order_api.stop_loss_order_id
                        self.insert_trade("SELL", self.current_candle.close, self.lot_size)
        ...
        if self.short_order and not self.stop_loss_order_id and not converting_to_buy and self.profit_taking_or_stop_loss_candle:
