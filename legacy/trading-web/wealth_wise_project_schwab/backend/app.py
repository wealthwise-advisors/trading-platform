...
STRATEGY_BOT_MAPPING = {
    "Strategy_One": "schwab_trading_bot_one.py",
    "Strategy_Two": "schwab_trading_bot_two.py",
    "Strategy_Three": "schwab_trading_bot_three.py"
}
...
class Bot(db.Model):
    ...
    enable_trade_buying = db.Column(db.Boolean, default=False)
    enable_trade_shorting = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.TIMESTAMP, server_default=db.func.current_timestamp())
    ...
    with SessionLocal() as session:
        ...
        for bot in bots:
            ...
            result.append({
                ...
                "strategy": bot.strategy,
                "enable_trade_buying": bot.enable_trade_buying,
                "enable_trade_shorting": bot.enable_trade_shorting,
                "stop_loss_adjust": float(bot.stop_loss_adjust)
            })
...
def bot_action():
    ...
    with SessionLocal() as session:
        ...
        if action == "START":
        ...
        elif action in ["BULK_BUY", "BULK_SELL"]:
            if lot_size <= 0:
            ...
            session.query(Bot).filter(Bot.bot_id.in_(bot_ids)).update({"enable_trade_buying": True}, synchronize_session=False)
            session.commit()

            for bot_id in bot_ids:
                redis_client.publish(f"bot:{bot_id}", "ENABLE_TRADING_BUYING")  # Notify bot

            logging.info(f"Buy Trading ENABLED for bots {bot_ids}")
            return jsonify({"message": f"Live trading enabled for bots {bot_ids}"}), 200

        elif action == "DISABLE_TRADING_BUYING":
            session.query(Bot).filter(Bot.bot_id.in_(bot_ids)).update({"enable_trade_buying": False}, synchronize_session=False)
            session.commit()

            for bot_id in bot_ids:
                redis_client.publish(f"bot:{bot_id}", "DISABLE_TRADING_BUYING")  # Notify bot

            logging.info(f"Buy trading DISABLED for bots {bot_ids}")
            return jsonify({"message": f"Live trading disabled for bots {bot_ids}"}), 200

        elif action == "ENABLE_TRADING_SHORTING":
            session.query(Bot).filter(Bot.bot_id.in_(bot_ids)).update({"enable_trade_shorting": True}, synchronize_session=False)
            session.commit()

            for bot_id in bot_ids:
                redis_client.publish(f"bot:{bot_id}", "ENABLE_TRADING_SHORTING")  # Notify bot

            logging.info(f"Short Trading ENABLED for bots {bot_ids}")
            return jsonify({"message": f"Live trading enabled for bots {bot_ids}"}), 200

        elif action == "DISABLE_TRADING_SHORTING":
            session.query(Bot).filter(Bot.bot_id.in_(bot_ids)).update({"enable_trade_shorting": False}, synchronize_session=False)
            session.commit()

            for bot_id in bot_ids:
                redis_client.publish(f"bot:{bot_id}", "DISABLE_TRADING_SHORTING")  # Notify bot

            logging.info(f"Short trading DISABLED for bots {bot_ids}")
            return jsonify({"message": f"Live trading disabled for bots {bot_ids}"}), 200
...
def run_backtest():
    ...
    enable_trade_buying = data.get("enable_trade_buying")
    enable_trade_shorting = data.get("enable_trade_shorting")
    ...
    try:
        result = subprocess.run(
            [
                ...
                str(lot_size), str(stop_loss_adjust), output_file, interval, str(enable_trade_buying), str(enable_trade_shorting)
            ],
            check=True,
