...
class RSIStochExtremeStrategy(Strategy):
    ...
    def add_divergence_info(self, is_buy, pre_condition, post_condition, start_candle, end_candle, is_extreme, is_rsi_13, swing_used, pre_condition_only, only_rsi, is_artificial_divergence=False):
        ...
        if "09:57 AM" in self.current_candle.date:
    ...
    def check_is_converting_to_short_valid(self, converting_to_short):
        ...
        if "10:21 AM" in self.current_candle.date:
            print("Put Break Point")
        ...
        if self.buy_order and len(self.trades) == 0 and not converting_to_short and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.wave1_uptrend_3leg is not None and self.current_candle.close > self.wave1_uptrend_3leg['High']:
                if self.stop_loss < self.current_candle.close:
                    ...
                    self.buy(sl=self.stop_loss,  size=self.lot_size)
                    self.in_trade_candle = self.current_candle
                    self.profit_taking_or_stop_loss_candle = None
        ...
        if self.buy_order and len(self.trades) == 0 and not converting_to_short and self.profit_taking_or_stop_loss_candle:
    ...
    def check_is_converting_to_buy_valid(self, converting_to_buy, converting_to_short=False):
        ...
        if self.short_order and len(self.trades) == 0 and not converting_to_buy and self.close_due_to_stop_loss and not self.stop_loss_used:
            if self.stop_loss > self.current_candle.close:
                if self.wave1_downtrend_3leg is not None and self.current_candle.close < self.wave1_downtrend_3leg['Low']:
                    ...
                    self.sell(sl=self.stop_loss,  size=self.lot_size)
                    self.in_trade_candle = self.current_candle
                    self.profit_taking_or_stop_loss_candle = None
                    self.close_due_to_stop_loss = None
                    self.stop_loss_used = True
        ...
        if self.short_order and len(self.trades) == 0 and not converting_to_buy and self.profit_taking_or_stop_loss_candle:
            divergence_info = self.get_latest_pre_and_post_condition_divergence("SHORT")
            if divergence_info:
    ...
    def next(self):
        ...
        if self.data.TRADE[-1] == 1:
            ...
            if self.buy_order or self.short_order:
                if len(self.trades) == 0 and not self.profit_taking_or_stop_loss_candle:
            ...
            for i in range(0, 2):
                ...
                if not self.in_trade and buy_condition:
                    ...
                    stop_limit_price = stop_loss - 300
