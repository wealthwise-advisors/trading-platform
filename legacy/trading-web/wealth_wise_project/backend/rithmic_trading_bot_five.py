...
# Configure logging
LOG_FILE = "/home/ubuntu/logs/trading_bot.log"
...
logging.basicConfig(
    ...
    force=True,   # <= important if anything configured logging before
)

# ----------- Important log only -----------
IMPORTANT_LOG_FILE = "/home/ubuntu/logs/trading_bot_important.log"
os.makedirs(os.path.dirname(IMPORTANT_LOG_FILE), exist_ok=True)

important_formatter = logging.Formatter(
    "%(asctime)s - %(levelname)s - %(message)s"
)

important_handler = logging.FileHandler(IMPORTANT_LOG_FILE)
important_handler.setLevel(logging.INFO)
important_handler.setFormatter(important_formatter)

important_logger = logging.getLogger("important_trades")
important_logger.setLevel(logging.INFO)
important_logger.addHandler(important_handler)
important_logger.propagate = False   # don't also write to common log
# -----------------------------------------
...
class TradingBot:
        ...
        for message in pubsub.listen():
            if message["type"] == "message":
                ...
                if action == "ENABLE_LIVE_TRADING":
                    ...
                    if self.buy_order and not self.short_order and not self.stop_loss_order :
                    ...
                    if self.short_order and not self.buy_order and not self.stop_loss_order:
    ...
    def log_helper_messages_for_trade(self):
        if not self.buying_shorting_conditions:
            return

        important_logger.info("===== Buy/Sell Conditions =====")

        for index, msg_list in self.buying_shorting_conditions.items():
            # Ensure we have a Timestamp
            try:
                ts = index.to_pydatetime() if hasattr(index, "to_pydatetime") else index
            except Exception:
                # Fallback if index is a string or something else
                import pandas as pd
                ts = pd.to_datetime(index)

            # Format as HH:MM:SS (change to "%H:%M" if you want only hours:minutes)
            ts_str = ts.strftime("%H:%M:%S")

            for msg in msg_list:
                # Timestamp in front of the message
                important_logger.info("%s - %s", ts_str, msg)

        important_logger.info("===== End Conditions =====")
        important_logger.info("")
    ...
    def run(self):
        ...
        while running:
            try:
                ...
                self.initial_df["RSI"] = talib.RSI(self.initial_df['Close'], 2)
                self.initial_df["RSI"] = self.initial_df["RSI"].round(2)
                logging.info(f"Bot Id {self.bot_id}, Data shape before apply # {data.shape}")
                ...
                for i in range(0, 2):
                    ...
                    if not self.in_trade and buy_condition:
                        try:
                            important_logger.info(f"{GREEN}{self.symbol_ironbeam} # Live Trading : {self.live_trading} #  Buy Order Reqeust In Progress{RESET}")
                            self.log_helper_messages_for_trade()
                    ...
                    if not self.in_trade and short_condition:
                        try:
                            important_logger.info(f"{RED}{self.symbol_ironbeam} # Live Trading : {self.live_trading} # Short Order Reqeust In Progress{RESET}")
                            self.log_helper_messages_for_trade()
                            stop_limit_price = float(stop_loss) + float(self.stop_loss_adjust)
