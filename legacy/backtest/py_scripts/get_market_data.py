...
class MarketDataHelper:
    ...
    # def fetch_price_history(self, symbol, date_str, end_date=None, interval=None):
    #     """
    ...
    #         # Convert date string to datetime and localize to CST
    #         specific_date_end = datetime.strptime(end_date, "%Y-%m-%d")
    #         specific_date_end_cst = self.cst.localize(specific_date_end)

    #         # Define start and end times (7:30 AM - 1:45 PM CST)
    #         start_time_cst = specific_date_cst.replace(hour=7, minute=30)
    #         end_time_cst = specific_date_end_cst.replace(hour=13, minute=45)
